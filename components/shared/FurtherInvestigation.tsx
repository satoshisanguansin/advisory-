...
import { streamInvestigation, generateFollowUpQuestions } from '../../services/geminiService';
...
const FurtherInvestigation: React.FC<FurtherInvestigationProps> = ({ sectionContext, sectionTitle, onNewContribution, report }) => {
  ...
  const handleStream = async (currentHistory: ChatMessage[]) => {
    ...
    try {
        ...
        // Proactively generate follow-up questions
        setIsGeneratingSuggestions(true);
        const finalHistory: Content[] = [...geminiContents, { role: 'model', parts: [{ text: fullResponse }] }];
        const suggestions = await generateFollowUpQuestions(finalHistory);
  ...
  const handleFollowUp = async (followUpQuestion: string) => {
    ...
    if (!followUpQuestion.trim() || isGenerating) return;
    
    const userMessage: ChatMessage = { role: 'user', content: followUpQuestion };
 ...
 const handleAddFinding = (modelMessage: ChatMessage) => {
    ...
    if (modelMessageIndex > 0) {
        for (let i = modelMessageIndex - 1; i >= 0; i--) {
            if (chatHistory[i].role === 'user' && !chatHistory[i].hidden) {
    ...
    <div className="mt-4 p-4 bg-black/30 rounded-lg border border-zinc-700/70">
       ...
       <div ref={chatContainerRef} className="max-h-80 overflow-y-auto space-y-4 pr-2 mb-3 border-b border-zinc-800 pb-3">
                    ...
                    <div className={`flex items-start gap-2.5 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        ...
                        <div className={`flex flex-col w-full max-w-prose leading-1.5 p-3 rounded-xl ${msg.role === 'user' ? 'bg-blue-600' : 'bg-zinc-800'}`}>
                            <div className="text-sm font-normal text-white whitespace-pre-wrap font-sans">
                                {isGenerating && isLastMessage && msg.role === 'model' ? (
                                    <>
                                        {msg.content}
                     ...
                     {showActions && (
                        <div className="flex items-center justify-start mt-1.5 ml-10 space-x-2">
                           {onNewContribution && <button
                                ...
                                disabled={isAdded}
                                className={`flex items-center gap-1.5 text-xs font-semibold px-2 py-1 rounded-md ${isAdded ? 'bg-green-500/20 text-green-400 cursor-default' : 'bg-zinc-700/50 text-gray-300 hover:bg-zinc-700'}`}>
                            ...
                            <button onClick={() => handleOpenScheduler(msg.content)} className="flex items-center gap-1.5 text-xs font-semibold px-2 py-1 rounded-md bg-zinc-700/50 text-gray-300 hover:bg-zinc-700">
                            ...
                            <button onClick={() => handleCopyToClipboard(msg.content)} className="flex items-center gap-1.5 text-xs font-semibold px-2 py-1 rounded-md bg-zinc-700/50 text-gray-300 hover:bg-zinc-700">
                                {isCopied ? <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 text-green-400" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg> : <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor"><path d="M7 9a2 2 0 012-2h6a2 2 0 012 2v6a2 2 0 01-2 2H9a2 2 0 01-2-2V9z" /><path d="M5 3a2 2 0 00-2 2v6a2 2 0 002 2V5h8a2 2 0 00-2-2H5z" /></svg>}
            ...
            {!isGenerating && suggestedQuestions.length > 0 && (
                <div className="ml-10 mt-3 space-y-2">
                    <h6 className="text-xs font-bold text-gray-400">คำถามที่แนะนำ:</h6>
                    <div className="flex flex-wrap gap-2">
                        {suggestedQuestions.map((q, i) => (
                            <button key={i} onClick={() => handleFollowUp(q)} className="px-3 py-1.5 bg-zinc-700/50 hover:bg-zinc-700 text-gray-300 hover:text-white rounded-full text-xs font-semibold transition-colors">
                                {q}
                            </button>
                        ))}
                    </div>
            ...
            {isGeneratingSuggestions && <div className="ml-10 mt-2 flex items-center gap-2"><LoadingSpinner className="w-4 h-4" /><span className="text-xs text-gray-500">กำลังสร้างคำถามแนะนำ...</span></div>}
        </div>
      ...
      <div className="flex items-center gap-2">
        <textarea
            ...
            onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleFollowUp(question); } }}
            placeholder={isLoggedIn ? "ถามคำถามต่อเนื่อง หรือแสดงความคิดเห็นในรายละเอียด..." : "กรุณาเข้าสู่ระบบเพื่อเริ่มการสืบค้น..."}
            className="w-full bg-[#212529] border border-gray-600 rounded-lg p-2 text-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500 transition-shadow resize-none max-h-40 overflow-y-auto"
            rows={1}
            disabled={isGenerating || !isLoggedIn}
        />
        <button aria-label="Send follow-up" onClick={() => handleFollowUp(question)} disabled={isGenerating || !question.trim()} className="p-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      {!isLoggedIn && (
        <div className="text-center mt-3">
            <button onClick={signIn} className="text-xs text-cyan-400 hover:underline font-semibold">เข้าสู่ระบบเพื่อใช้งาน AI</button>
        </div>
      )}

      {isScheduling && <ScheduleEventModal
          isOpen={isScheduling}
